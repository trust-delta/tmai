...
cargo run -- --audit   # 検出監査ログ有効（/tmp/tmai/audit/detection.ndjson）
...
`/tmp/tmai/audit/detection.ndjson`（10MB超過で `.ndjson.1` にローテーション）
...
```bash
# 直近の状態変化を確認
cat /tmp/tmai/audit/detection.ndjson | jq 'select(.event == "StateChanged")'

# 低確信度の検出を抽出（改善候補）
cat /tmp/tmai/audit/detection.ndjson | jq 'select(.reason.confidence == "Low")'

# 特定ペインの履歴
cat /tmp/tmai/audit/detection.ndjson | jq 'select(.pane_id == "5")'

# 検出漏れの可能性を確認（Processing中にユーザ入力）
cat /tmp/tmai/audit/detection.ndjson | jq 'select(.event == "UserInputDuringProcessing")'
```
