// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.
import type { ActionOrigin } from "./ActionOrigin";
import type { BundleId } from "./BundleId";
import type { BundleStatus } from "./BundleStatus";
import type { CapacityCauseSummary } from "./CapacityCauseSummary";
import type { DetectionSource } from "./DetectionSource";
import type { DispatchId } from "./DispatchId";
import type { DispatchIntentSummary } from "./DispatchIntentSummary";
import type { DispatchRefs } from "./DispatchRefs";
import type { DispatchState } from "./DispatchState";
import type { EntityChange } from "./EntityChange";
import type { ErrorCode } from "./ErrorCode";
import type { GuardrailKind } from "./GuardrailKind";
import type { QueuedPrompt } from "./QueuedPrompt";
import type { TmaiError } from "./TmaiError";
import type { VendorAvailabilityState } from "./VendorAvailabilityState";
import type { WorktreeInfo } from "./WorktreeInfo";

/**
 * Events emitted by the core when state changes occur.
 *
 * Consumers call [`TmaiCore::subscribe()`] to receive these events
 * via a `broadcast::Receiver`.
 *
 * This enum is the single source of truth for SSE event shapes —
 * the TypeScript discriminated union in
 * `crates/tmai-app/web/src/types/generated/CoreEvent.ts` is generated
 * from this definition by ts-rs (#446). Do not edit that file by hand.
 */
export type CoreEvent = { "type": "AgentsUpdated" } | { "type": "AgentStatusChanged", 
/**
 * Agent target ID
 */
target: string, 
/**
 * Previous status description
 */
old_status: string, 
/**
 * New status description
 */
new_status: string, } | { "type": "AgentAppeared", 
/**
 * Agent target ID
 */
target: string, } | { "type": "AgentDisappeared", 
/**
 * Agent target ID
 */
target: string, } | { "type": "TeamsUpdated" } | { "type": "TeammateIdle", 
/**
 * Agent target ID
 */
target: string, 
/**
 * Team name
 */
team_name: string, 
/**
 * Member name
 */
member_name: string, } | { "type": "TaskCreated", 
/**
 * Team name
 */
team_name: string, 
/**
 * Task ID
 */
task_id: string, 
/**
 * Task subject
 */
task_subject: string, } | { "type": "TaskCompleted", 
/**
 * Team name
 */
team_name: string, 
/**
 * Task ID
 */
task_id: string, 
/**
 * Task subject
 */
task_subject: string, } | { "type": "ConfigChanged", 
/**
 * Agent target ID
 */
target: string, 
/**
 * Config source (e.g., "user_settings", "project_settings")
 */
source: string, 
/**
 * Changed file path
 */
file_path: string, } | { "type": "WorktreeCreated", 
/**
 * Agent target ID
 */
target: string, 
/**
 * Worktree details (name, path, branch, original_repo)
 */
worktree: WorktreeInfo | null, } | { "type": "WorktreeRemoved", 
/**
 * Agent target ID
 */
target: string, 
/**
 * Worktree details (name, path, branch, original_repo)
 */
worktree: WorktreeInfo | null, } | { "type": "InstructionsLoaded", 
/**
 * Agent target ID
 */
target: string, } | { "type": "AgentStopped", 
/**
 * Agent target ID
 */
target: string, 
/**
 * Working directory
 */
cwd: string, 
/**
 * Last assistant message (if available)
 */
last_assistant_message: string | null, } | { "type": "ContextCompacting", 
/**
 * Agent target ID
 */
target: string, 
/**
 * How many compactions have occurred in this session
 */
compaction_count: number, } | { "type": "WorktreeSetupCompleted", 
/**
 * Absolute path to the worktree
 */
worktree_path: string, 
/**
 * Branch name
 */
branch: string, } | { "type": "WorktreeSetupFailed", 
/**
 * Absolute path to the worktree
 */
worktree_path: string, 
/**
 * Branch name
 */
branch: string, 
/**
 * Error message
 */
error: string, } | { "type": "PromptReady", 
/**
 * Agent target ID
 */
target: string, 
/**
 * The prompt text to send
 */
prompt: string, } | { "type": "PromptQueueChanged", 
/**
 * Agent target ID
 */
target: string, 
/**
 * Remaining queue contents after the mutation (oldest-first)
 */
queue: Array<QueuedPrompt>, } | { "type": "UsageUpdated" } | { "type": "ToolCallDeferred", 
/**
 * Unique deferred call ID
 */
defer_id: bigint, 
/**
 * Agent target/pane ID
 */
target: string, 
/**
 * Tool name
 */
tool_name: string, } | { "type": "RebaseSucceeded", 
/**
 * Branch that was rebased
 */
branch: string, 
/**
 * Worktree path
 */
worktree_path: string, } | { "type": "RebaseConflict", 
/**
 * Branch that had conflicts
 */
branch: string, 
/**
 * Worktree path
 */
worktree_path: string, 
/**
 * Error details
 */
error: string, } | { "type": "ToolCallResolved", 
/**
 * Unique deferred call ID
 */
defer_id: bigint, 
/**
 * Agent target/pane ID
 */
target: string, 
/**
 * Resolution: "allow" or "deny"
 */
decision: string, 
/**
 * Who resolved it (e.g., "human", "ai:haiku", "timeout")
 */
resolved_by: string, } | { "type": "PrCreated", 
/**
 * PR number
 */
pr_number: bigint, 
/**
 * PR title
 */
title: string, 
/**
 * Head branch name
 */
branch: string, } | { "type": "PrCiPassed", 
/**
 * PR number
 */
pr_number: bigint, 
/**
 * PR title
 */
title: string, 
/**
 * Summary of passed checks
 */
checks_summary: string, } | { "type": "PrCiFailed", 
/**
 * PR number
 */
pr_number: bigint, 
/**
 * PR title
 */
title: string, 
/**
 * Details of failed checks
 */
failed_details: string, } | { "type": "PrReviewFeedback", 
/**
 * PR number
 */
pr_number: bigint, 
/**
 * PR title
 */
title: string, 
/**
 * Summary of review comments
 */
comments_summary: string, 
/**
 * Monotonic review count from PR state — distinguishes
 * back-to-back ChangesRequested transitions so the notifier
 * can dedupe on `(pr_number, review_count)` instead of
 * collapsing distinct rounds into one entry.
 */
review_count: bigint, } | { "type": "PrClosed", 
/**
 * PR number
 */
pr_number: bigint, 
/**
 * PR title
 */
title: string, 
/**
 * Head branch name
 */
branch: string, } | { "type": "GitStateChanged", 
/**
 * Repository directory path
 */
repo: string, } | { "type": "GuardrailExceeded", 
/**
 * The type of guardrail that was exceeded
 */
guardrail: GuardrailKind, 
/**
 * Associated branch
 */
branch: string, 
/**
 * Associated PR number (if any)
 */
pr_number: bigint | null, 
/**
 * Current count that exceeded the limit
 */
count: bigint, 
/**
 * The configured limit
 */
limit: bigint, } | { "type": "AgentTargetChanged", 
/**
 * Previous target ID (e.g., "main:0.2")
 */
old_target: string, 
/**
 * New target ID (e.g., "main:0.1")
 */
new_target: string, 
/**
 * Process ID that links the two targets
 */
pid: number, } | { "type": "ActionPerformed", 
/**
 * Who initiated the action
 */
origin: ActionOrigin, 
/**
 * API action name (e.g., "dispatch_issue", "kill_agent", "merge_pr")
 */
action: string, 
/**
 * Human-readable summary of the action
 */
summary: string, } | { "type": "DispatchRejected", 
/**
 * Redacted description of what was dispatched.
 */
intent_summary: DispatchIntentSummary, 
/**
 * Who issued the rejected dispatch.
 */
origin: ActionOrigin, 
/**
 * Structured reason for the rejection.
 */
error: TmaiError, } | { "type": "VendorAvailabilityChanged", 
/**
 * Vendor identifier (e.g. "anthropic", "openai", "google").
 */
vendor: string, 
/**
 * Account label, if the deployment distinguishes accounts.
 */
account?: string, 
/**
 * Previous availability state.
 */
old: VendorAvailabilityState, 
/**
 * New availability state.
 */
new: VendorAvailabilityState, 
/**
 * How the transition was detected (hook, polling, explicit signal).
 */
detected_via: DetectionSource, } | { "type": "CapacityChanged", 
/**
 * New value of the in-use slot counter.
 */
current: number, 
/**
 * Configured ceiling at the time of emission.
 */
limit: number, 
/**
 * Signed change applied to `current` by this event
 * (+1 on spawn, -1 on terminal, 0 on `LimitChanged`/`Reconciled`).
 */
delta: number, 
/**
 * What triggered the change.
 */
cause: CapacityCauseSummary, } | { "type": "ContractViolation", 
/**
 * Origin of the violating call.
 */
origin: ActionOrigin, 
/**
 * Machine-readable classification of the violation.
 */
code: ErrorCode, 
/**
 * Code-specific structured context (never includes secrets or
 * raw prompts). Defaults to `null` when the emitter has nothing
 * to attach.
 */
context: unknown, } | { "type": "DispatchBypassUsed", 
/**
 * Origin of the bypassing call.
 */
origin: ActionOrigin, 
/**
 * Names of the bypassed checks / stages (e.g.
 * "capacity_check", "vendor_availability").
 */
bypassed: Array<string>, 
/**
 * Free-text justification supplied by the caller.
 */
reason: string, } | { "type": "BundleStatusChanged", 
/**
 * Identifier of the bundle whose status changed.
 */
bundle_id: BundleId, 
/**
 * Previous rollup status.
 */
old: BundleStatus, 
/**
 * New rollup status.
 */
new: BundleStatus, } | { "type": "DispatchSpawned", 
/**
 * Durable id assigned at creation time.
 */
dispatch_id: DispatchId, 
/**
 * Stable references captured at request time.
 */
refs: DispatchRefs, } | { "type": "DispatchStateChanged", 
/**
 * Dispatch whose state changed.
 */
dispatch_id: DispatchId, 
/**
 * Previous state.
 */
old: DispatchState, 
/**
 * New state.
 */
new: DispatchState, } | { "type": "DispatchStopped", 
/**
 * Dispatch that finished.
 */
dispatch_id: DispatchId, 
/**
 * Free-text reason (e.g. "agent_exited", "killed_by_user",
 * "spawn_failed: tmux error").
 */
reason: string, } | { "type": "AgentUpdate", 
/**
 * Whether the agent was upserted or removed.
 */
change: EntityChange, 
/**
 * Stable agent identifier (UUID short hash).
 */
id: string, 
/**
 * Serialized [`super::types::AgentSnapshot`]; `None` when removed.
 */
snapshot?: unknown, 
/**
 * Who or what triggered this update.
 */
origin: ActionOrigin | null, } | { "type": "DispatchUpdate", 
/**
 * Whether the dispatch was upserted or removed.
 */
change: EntityChange, 
/**
 * Hyphenated dispatch id string.
 */
id: string, 
/**
 * Serialized [`super::snapshot::DispatchSnapshot`]; `None` when removed.
 */
snapshot?: unknown, 
/**
 * Who or what triggered this update.
 */
origin: ActionOrigin | null, } | { "type": "TeamUpdate", 
/**
 * Whether the team was upserted or removed.
 */
change: EntityChange, 
/**
 * Team name (primary key).
 */
name: string, 
/**
 * Serialized [`super::types::TeamSnapshot`]; `None` when removed.
 */
snapshot?: unknown, 
/**
 * Who or what triggered this update.
 */
origin: ActionOrigin | null, } | { "type": "WorktreeUpdate", 
/**
 * Whether the worktree was upserted or removed.
 */
change: EntityChange, 
/**
 * Worktree path (unique per repo).
 */
id: string, 
/**
 * Serialized [`super::types::WorktreeSnapshot`]; `None` when removed.
 */
snapshot?: unknown, 
/**
 * Who or what triggered this update.
 */
origin: ActionOrigin | null, } | { "type": "QueueUpdate", 
/**
 * Serialized full [`super::types::QueueSnapshot`] after the mutation.
 */
snapshot: unknown, 
/**
 * Who or what triggered this update.
 */
origin: ActionOrigin | null, } | { "type": "RuntimeUpdate", 
/**
 * Serialized full [`super::snapshot::RuntimeSnapshot`].
 */
snapshot: unknown, 
/**
 * Who or what triggered this update.
 */
origin: ActionOrigin | null, };
