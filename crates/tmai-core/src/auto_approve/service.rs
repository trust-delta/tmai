        ...
        loop {
            ...
            let candidates = {
                ...
                for target in &targets {
                    ...
                    if is_genuine_user_question(&approval_type, interaction.as_ref()) {
...
/// Check if an approval is a genuine user question (not a standard permission prompt).
///
...
fn is_genuine_user_question(
    ...
    if *approval_type != ApprovalCategory::UserQuestion {
        return false;
    }
    ...
    match interaction {
        ...
        // No interaction info = hook-based detection without choice info → genuine
        None => true,
    }
}
...
mod tests {
    ...
    fn test_is_genuine_user_question_multi_select() {
        ...
        let interaction = InteractionMode::MultiSelect {
            choices: vec!["Yes".to_string(), "No".to_string()],
            cursor_position: 1,
        };
        assert!(is_genuine_user_question(
    ...
    fn test_is_genuine_user_question_non_user_question() {
        ...
        assert!(!is_genuine_user_question(&ApprovalCategory::FileEdit, None));
        assert!(!is_genuine_user_question(
    ...
    fn test_is_genuine_user_question_empty_choices() {
        // No interaction info from hook-based detection → genuine (requires human judgment)
        assert!(is_genuine_user_question(
            &ApprovalCategory::UserQuestion,
            None,
        ));
