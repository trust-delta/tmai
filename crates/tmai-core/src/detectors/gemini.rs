...
use crate::agents::{AgentMode, AgentStatus, AgentType, ApprovalCategory, InteractionMode};
...
impl GeminiDetector {
    ...
    fn detect_content_approval(
        &self,
        content: &str,
    ) -> Option<(ApprovalCategory, String, &str, Option<InteractionMode>)> {
...
impl StatusDetector for GeminiDetector {
    ...
    fn detect_status_with_reason(
        ...
        if title.contains(TITLE_ACTION_REQUIRED_ICON) {
            ...
            if let Some((approval_type, details, rule, interaction)) =
