...
WebUIダッシュボードでは、チームがサイドバーにエージェントと並んで表示されます。チームデータはSSEイベント経由でリアルタイムに更新されます：

- **`teams`** — チーム構造とタスクサマリーの更新
- **`teammate_idle`** — チームメンバーがアイドルになった通知
- **`task_completed`** — タスクが完了した通知

WebUIではチームメンバーがチーム名の下にグループ化され、タスク進捗インジケータが表示されます。
