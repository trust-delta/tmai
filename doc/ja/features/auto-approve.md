...
## 今後: Hook ベース自動承認（Defer Permission）

Claude Code v2.1.89で導入されるDefer Permission機能により、hookベースの自動承認が可能になります。従来の画面スクレイピング＋キー送信方式と比べ、構造化されたデータでの判定と確実な応答が実現します。

### 仕組み

```
Claude Codeがツール実行を試みる
  ↓
PreToolUse hookがtmaiに送信される
  ↓
tmaiが構造化データ（ツール名、パラメータ）で判定
  ├─ 承認 → hookレスポンスで "approve" を返す
  ├─ 拒否 → hookレスポンスで "deny" を返す
  └─ 保留 → hookレスポンスで "defer" を返す（ユーザーに委ねる）
```

### メリット

| 項目 | 従来方式（画面＋キー送信） | Hook ベース（Defer Permission） |
|------|-------------------------|-------------------------------|
| **レイテンシ** | ポーリング間隔 + キー送信遅延 | 遅延ゼロ（同期レスポンス） |
| **データ形式** | 画面テキストの解析 | 構造化データ（JSON） |
| **送信確実性** | キー送信の失敗リスクあり | hookレスポンスで確実に伝達 |
| **フィルタリング** | テキストパターンマッチ | ツール名・パラメータによる条件付きフィルタリング |

詳細は [#176](https://github.com/user/tmai/issues/176) を参照してください。
