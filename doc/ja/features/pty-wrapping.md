...
手動起動したClaude Code（非IPC）を、会話コンテキストを保持したままPTYラッピングにアップグレードできます：

1. tmaiで非IPCエージェントを選択
2. `W`（Shift+W）を押す
3. `.jsonl` ファイルからセッションIDを特定
4. 確認ダイアログで承認 → `claude --resume` + `tmai wrap` で再起動

セッションID特定は2段構え：
- **Phase 1**: capture-pane内容をJSONLファイルと照合（非侵襲）
- **Phase 2**: マーカーを送信して特定（会話履歴に1ターン分の痕跡が残る）
- **両方失敗時**: エラーメッセージが表示されます。リトライするか、パススルーモードで手動再起動してください。
