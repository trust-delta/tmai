...
並列開発用のワークツリーを作成する方法は2つあります：

### 推奨: `dispatch_issue`（MCP）

最もシンプルなアプローチ — MCPツール1回の呼び出しで全てが完了：

```
dispatch_issue(issue_number: 42)
  → GitHub Issue #42を取得
  → .claude/worktrees/42-feat-description/ にワークツリーを作成
  → Issueコンテキストをプロンプトとしてエージェントを起動
```

詳細は[Issue駆動オーケストレーション](./issue-driven-orchestration.md)を参照。

### 手動: `git worktree` + `claude`

完全にコントロールしたい場合は、ワークツリーを手動で作成：
...
オーケストレーターから `delete_worktree` MCPツールを使用：

```
delete_worktree(worktree_name: "42-feat-description")
  → ワークツリーディレクトリを削除し、git参照をプルーニング
```

未コミットの変更がある場合は `force: true` を使用。
...
UIまたはMCPツールで作成されたワークツリーはリポジトリ内の`.claude/worktrees/<name>/`配下に配置されます。

詳細は[ワークツリー管理](../features/worktree-ui.md)を参照。

## tmaiの思想

tmaiは複数のアプローチをサポートします：

- **MCPツール**: `dispatch_issue` と `spawn_worktree` によるプログラマティックなオーケストレーション
...
- [Issue駆動オーケストレーション](./issue-driven-orchestration.md) - MCP駆動のオーケストレーターワークフロー
