...
- 2026-05-25 (currency sweep, verified): the left-panel successor stance ([[2026-05-23-producer-rooted-left-panel]]) landed via PR #727 (`cf4f65c`) + #729 (`7c548e5`); dead orchestrator-guardrail UI was removed via PR #728 (`5030161`), consistent with this DR's Fork-B legacy-orchestrator retire direction. No conflict with this DR's stance; `last-verified` bumped.
...
- 2026-05-29 (currency sweep, partial-supersede acknowledged): PRs #745 (`b3bf54e`) + #746 (`f32aeba`) landed reshaping the right column. Lifecycle wire (#745) surfaces all approach statuses (`Active` retired into `Ready` + `Running`); R panel (#746) redefines the right surface as the **vendor-neutral project-artifact inventory** per [[2026-05-29-r-panel-as-project-artifact-inventory]]: 7 collapsed sections (PRs / Issues / Decisions / Approaches / Calibration / Hand-over / Files), operator-driven accordion, no tmai-side appraisal. This **partial-supersedes** the 2026-05-22 Refinement *Fork A "dumb status surface"* framing AND the 2026-05-23 *P1.1 "strip-content rule (changes-during-session = strip)"* — R now carries every artifact, not just attention. What holds: the rebuild-as-Producer-console commitment, the L/C/R co-visible layout, the central hand-over digest, and the legacy-retain stance for emergency override paths. `last-verified` bumped.
- 2026-06-05 (currency sweep, verified): the attention-model §3 work landed on `clients/react/` — PR #768/#769 (`58b8ec0`) added per-artifact attention markers + authorship-scoped colouring (new `r-panel/AttentionMarker.tsx`, `hooks/useUnitAttention.ts`, R{Prs,Issues,Decisions,Approaches}Section wiring, `lib/api-http.ts`/`api-tauri.ts` client + `globals.css`), and PR #772/#773 (`4d37a4e`) reshaped §3-2b by removing the Δ-stream / Calibration / Hand-over **sections** from the R panel (agent-operation surfaces, not project artifacts). Both are R-panel evolution consistent with the 2026-05-29 partial-supersede (R = vendor-neutral artifact inventory): attention is now a per-artifact field on each artifact rather than a standalone section. What holds is unchanged — the rebuild-as-Producer-console commitment, the L/C/R co-visible layout, the central hand-over digest (the removed R-panel Hand-over *section* was a duplicate artifact view, not the centre digest), and the emergency-override legacy-retain stance. `last-verified` bumped.
- 2026-06-07 (currency sweep, verified): the aim-tree graduation work landed on `clients/react/` — #782/#783 (`f279828`) reshaped the aim-tree read view into a maximized portal canvas + variable-height layout, #785 (`3934664`, Stage 2-B) wired the write affordances (edit-frontmatter / create-node) into `RAimsSection` via `useUnitAims.ts` + `lib/api-http.ts`/`api-tauri.ts` against the new tmai-core PUT/POST endpoints (#502), and #784 (`ea5d5c4`) synced the generated `Aim{Edit,Create}Request` types. All are R-panel evolution consistent with the 2026-05-29 partial-supersede (R = vendor-neutral artifact inventory): the aim-tree is the write-first artifact section, now read+write. What holds is unchanged — the rebuild-as-Producer-console commitment, the L/C/R co-visible layout, the central hand-over digest, and the emergency-override legacy-retain stance. `last-verified` bumped.
