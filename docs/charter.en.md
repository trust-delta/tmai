...
- **Purpose and means are kept apart.** Where a piece of work is *going* — the outcome you commit to, the thing you'll bear if it's wrong — is written by you, and kept distinct from *how it is currently being pursued*, which the Producer maintains and may rewrite at any time. A means never quietly becomes the commitment. The apparatus that currently holds both is [Aim](aim.en.md) — a means itself, and replaceable.
...
- What this repo carries is therefore thin: **this charter** (the purpose) and **[Aim](aim.en.md)** (the means that purpose currently reaches for). Both are public-appropriate — they are the *what*, never the engine's *how*. Everything under [`docs/archive/`](archive/) is a retired audit trail, not current direction.
