...
- 2026-06-07 (currency re-verify, no change): aim-tree graduation の `clients/react/` 変更（#782/#783 `f279828` maximized canvas + 可変高 layout、#785 `3934664` Stage 2-B write affordances = `RAimsSection`/`useUnitAims.ts`/`api-http`/`api-tauri` の edit/create 配線）が governs-path 粒度で drift flag を立てたが、re-verify の結果 **orthogonal** と確認 — いずれも R-panel の aim-tree section（artifact 面）の read→write 拡張で、本 posture の load-bearing site（`useHandover.ts` の missing-precondition 検出 + `TODO(tmai-core#341)`、`SettledDecisionsSection.tsx` の `TODO(tmai-core#340)` notice）には未接触。posture compensation と row 2 の `noLiveAgents` markers は不変、`#340`/`#341` とも OPEN 据え置き。Decision hold、`status: accepted` 据え置き、`last-verified` のみ bump。
- 2026-06-05 (currency re-verify, no change): attention-model §3 の `clients/react/` 変更（#768/#769 `58b8ec0` per-artifact attention markers、#772/#773 `4d37a4e` §3-2b reshape）が governs-path 粒度で drift flag を立てたが、re-verify の結果 **orthogonal** と確認 — 両 commit の touched files は全て R-panel（artifact 面）の attention marker 追加・section 除去・attention hook/client/css で、本 posture の load-bearing site（`useHandover.ts` の missing-precondition 検出 + `TODO(tmai-core#341)`、`SettledDecisionsSection.tsx` の `TODO(tmai-core#340)` notice）には未接触（#773 が消したのは R-panel の `RHandoverViewer`/`RHandoverSection` であって `useHandover.ts` hook ではない）。posture compensation と row 2 の `noLiveAgents` markers は不変。Decision hold、`status: accepted` 据え置き、`last-verified` のみ bump。
